...
import { ArrowRight, CheckCircle, Truck, ShieldCheck, Star } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-16 pb-16">
      ...
      <section className="container mx-auto px-4 max-w-7xl py-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div className="space-y-4">
            <h2 className="text-4xl font-bold text-text-heading font-serif">Featured Collection</h2>
          ...
          <Link href="/products" className="hidden md:block">
            <Button variant="ghost" className="text-primary hover:text-primary-dark group text-lg font-medium">
        ...
        <div className="mt-12 text-center md:hidden">
          <Link href="/products">
            <Button variant="outline" className="w-full h-12 text-lg">View All Products</Button>
          </Link>
        </div>
      </section>
      ...
      <section className="bg-white py-24 relative overflow-hidden">
        {/* Decorative elements */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-secondary-dark rounded-full mix-blend-multiply filter blur-3xl opacity-30 translate-x-1/2 -translate-y-1/2"></div>
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-accent-light rounded-full mix-blend-multiply filter blur-3xl opacity-30 -translate-x-1/2 translate-y-1/2"></div>
        ...
        <div className="container mx-auto px-4 max-w-7xl grid md:grid-cols-2 gap-16 items-center relative z-10">
          <div className="relative h-[500px] rounded-2xl overflow-hidden shadow-2xl">
            ...
            />
            <div className="absolute bottom-6 left-6 right-6 bg-white/90 backdrop-blur-md p-6 rounded-xl shadow-lg border border-white/50">
              <div className="flex items-center gap-4">
                <div className="bg-primary/10 p-3 rounded-full">
                  <Star className="h-6 w-6 text-primary fill-primary" />
                </div>
                <div>
                  <p className="text-sm text-text-muted uppercase tracking-wider font-bold">Trusted Legacy</p>
                  <p className="text-text-heading font-serif text-lg">Over 30 Years of Excellence</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <h2 className="text-5xl font-bold text-text-heading font-serif leading-tight">Preserving the Art <br /> of <span className="text-primary italic">Kerala Handloom</span></h2>
            ...
            <div className="pt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="flex items-center gap-3 bg-secondary p-4 rounded-lg">
                  <CheckCircle className="h-5 w-5 text-gold" />
                  <span className="font-medium text-text-heading">{item}</span>
                </div>
              ))}
            </div>
            ...
            <div className="pt-4">
              <Link href="/contact">
                <Button variant="outline" className="border-primary text-primary hover:bg-primary hover:text-white h-12 px-8">Read More About Us</Button>
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="container mx-auto px-4 max-w-7xl py-12">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
          <h2 className="text-4xl font-bold text-text-heading font-serif">Why Choose Khadi Vasthra?</h2>
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    <div className="group bg-white p-8 rounded-2xl shadow-sm border border-secondary-dark text-center hover:shadow-xl hover:border-gold/30 transition-all duration-300 transform hover:-translate-y-2">
      <div className="flex justify-center mb-6 bg-secondary group-hover:bg-primary/5 p-5 rounded-full w-20 h-20 mx-auto items-center transition-colors">
        {icon}
      </div>
      <h3 className="text-xl font-bold mb-4 text-text-heading font-serif">{title}</h3>
