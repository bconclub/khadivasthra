    ...
    return (
        <div className={`bottom-bar fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white/60 backdrop-blur-xl border-t border-white/30 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] transition-all duration-300 ${isHome ? 'animate-slide-up' : ''}`}>
            <nav className="flex items-center justify-around h-16 px-1">
                ...
                <Link
                    href="/shop"
                    className={`bottom-bar__item flex flex-col items-center justify-center gap-0.5 flex-1 py-1 transition-colors ${pathname?.startsWith("/shop") ? "text-coral" : "text-gray-500 hover:text-gray-800"
                        }`}
                >
                ...
                <Link
                    href="/shop"
                    className={`bottom-bar__item flex flex-col items-center justify-center gap-0.5 flex-1 py-1 text-gray-500 hover:text-gray-800 transition-colors`}
                >
