...
CREATE POLICY IF NOT EXISTS "Public read product colors" ON product_colors FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Public read product variants" ON product_variants FOR SELECT USING (true);

-- Authenticated (admin) full access
CREATE POLICY IF NOT EXISTS "Admin manage product colors" ON product_colors FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Admin manage product variants" ON product_variants FOR ALL USING (auth.role() = 'authenticated');

-- Update decrement_stock function to support new variant system
