...
CREATE TABLE orders (
  ...
  status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','confirmed','billed','shipped','delivered','cancelled')),
  notes TEXT,
  ...
  settlement_status TEXT NOT NULL DEFAULT 'pending' CHECK (settlement_status IN ('pending','received','settled')),
